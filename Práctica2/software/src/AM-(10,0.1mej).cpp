...
// Algoritmo Genético Generacional para el MDP 
// (Operador de cruce basado en posición)
//
// Carlota Valdivia Manzano
// NOTA: compilar g++ AM-(10,0.1mej).cpp -o AM-(10,0.1mej)
//       ejecutar ./AM-(10,0.1mej) archivo.txt semilla
/////////////////////////////////////////////////////////////////
...
const float prob_bl = 0.1;          // Probabilidad de búsqueda local
...
// contribución
/*****************************************************************/
bool criterioOrdenacionPares(pair<int,float> & elem1, pair<int,float> & elem2) {
...
// Se ordena según la contribución de forma descendente
/*****************************************************************/
bool criterioOrdenacionSol(solucion & s1, solucion & s2) {
...
void poblacionAleatoria(int m, poblacion & p, vector<vector<float> > & matriz){
    ...
    for (unsigned int i = 0; i < p.tam; i++){
       solucionAletoria(m,p.C[i],matriz);
    }

    // Ordenamos la poblacion por contribución
    sort(p.C.begin(), p.C.end(), criterioOrdenacionSol);
}
...
void reemplazamiento(poblacion & pPadres, poblacion & pActual){
    ...
    pPadres.C[num_cromosomas-1] = pActual.C[0];
...
void actualizarContribuciones(poblacion & p, vector<vector<float> > & matriz){
    ...
    // Ordenamos según la contribución
    sort(p.C.begin(), p.C.end(), criterioOrdenacionSol);
}
...
void actualizarContribucionesBL(pair<int,float> & elemento_in, pair<int,float> & elemento_out, vector<pair<int,float>> & Sel, vector<vector<float> > & matriz){       
    ...
    sort(Sel.begin(),Sel.end(),criterioOrdenacionPares);
}
...
void memetico(int m, solucion & s, vector<vector<float> > & matriz){
    ...
    // Seleccionamos los nuevos valores
    for (unsigned j = 0; j < Sel.size(); j++){
...
void AM(int m, poblacion & pActual, vector<vector<float> > & matriz){
    ...
    int num_bl = prob_bl * num_cromosomas;  // Nº de búsquedas locales a realizar
    ...
    do {
        ...
        for (int i = 1; i < num_mutaciones+1; i++){
            ...
            // Escogemos los números haciendo módulos
...
int main(int argc, char *argv[]){
    ...
    //cout << endl << endl;
    //imprimirPoblacion(pActual);
    //imprimirSolucion(pActual.C[pActual.mejorSol]);
