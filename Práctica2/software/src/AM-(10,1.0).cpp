...
// Algoritmo Memético para el MDP 
//
...
/*****************************************************************/
void memetico(int m, poblacion & p, vector<vector<float> > & matriz){
    ...
    for (int i = 0; i < num_cromosomas; i++){
        ...
        // Pasamos de binaro a entero
        for (unsigned j = 0; j < p.C[i].sel.size(); j++){
        ...
        p.C[i].sel.clear();
        p.C[i].sel = vector<int> (matriz[0].size(),0);

        // Pasamos de entero a binario
        for (unsigned j = 0; j < Sel.size(); j++){

            p.C[i].sel[Sel[j].first] = 1;
        }

        // Señalamos que tenemos que actualizar su contribución
        p.C[i].actualizada = false;

        // Vaciamos el set y el vector
        S.clear();
        Sel.clear();
    }
}
